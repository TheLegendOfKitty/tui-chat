...
use crate::common::{
    read_data, send_with_header, Client, ClientList, ImageData, MessageType, Packet, PktSource,
    ReadResult,
};
...
const IMAGE_ZOOM_IN: Input = Input {
    key: Key::Char('+'),
    /*ctrl: true,*/ ctrl: false,
    alt: false,
    shift: false,
};
const IMAGE_ZOOM_OUT: Input = Input {
    key: Key::Char('-'),
    /*ctrl: true,*/ ctrl: false,
    alt: false,
    shift: false,
};
const _IMAGE_MOVE_LEFT: Input = Input {
    key: Key::Left,
    ctrl: false,
    alt: false,
    shift: false,
};
const _IMAGE_MOVE_RIGHT: Input = Input {
    key: Key::Right,
    ctrl: false,
    alt: false,
    shift: false,
};
const _IMAGE_MOVE_UP: Input = Input {
    key: Key::Up,
    ctrl: false,
    alt: false,
    shift: false,
};
const _IMAGE_MOVE_DOWN: Input = Input {
    key: Key::Down,
    ctrl: false,
    alt: false,
    shift: false,
};
const SHIFT_BACK: Input = Input {
    key: Key::Char('<'),
    ctrl: false,
    alt: false,
    shift: false,
};
const MESSAGE_SELECT: Input = Input {
    key: Key::Enter,
    ctrl: false,
    alt: false,
    shift: false,
};
const SHIFT_TO_PEERS: Input = Input {
    key: Key::Char('P'),
    ctrl: false,
    alt: false,
    shift: true,
};
const SHIFT_TO_MESSAGES_LIST: Input = Input {
    key: Key::Char('L'),
    ctrl: false,
    alt: false,
    shift: true,
    ...
    fn to_list(&self, width: usize) -> List<'a> {
        ...
        for i in 0..self.items.len() {
            ...
            match &packet.message_type {
                MessageType::STRING => {
                    for sub_string in
                        sub_strings(String::from_utf8(packet.data.clone()).unwrap(), width)
                            .iter()
                            .rev()
                    {
        ...
        let messages_list = List::new(messages_vec)
            .block(
                Block::default()
                    .title("Messages")
                    .borders(Borders::ALL)
                    .style(if self.state.selected().is_some() {
                        Style::default()
                    } else {
...
impl<'a> From<&Client> for ListItem<'a> {
    fn from(value: &Client) -> Self {
        ListItem::new(value.addr.to_string())
            .style(Style::default().fg(Color::White).bg(Color::Black))
    }
}
...
where
    for<'b> &'b T: Into<ListItem<'a>>, /* for more information on higher-ranked polymorphism, visit https://doc.rust-lang.org/nomicon/hrtb.html */
{
    fn from(value: &StatefulList<T>) -> Self {
        let mut vec: Vec<ListItem> = Vec::new();
        for item in &value.items {
            vec.push(<&T as Into<ListItem>>::into(item));
        }
        ...
        let list = List::new(vec)
            .block(
                Block::default()
                    ...
                    .borders(Borders::ALL)
                    .style(if value.state.selected().is_some() {
                        Style::default()
                    } else {
                        Style::default().fg(Color::DarkGray)
                    }),
...
impl FileBrowser {
    fn new(current_dir: PathBuf, picker: &Picker) -> Self {
        let mut current_dir_list = StatefulList::with_items(
                ...
                Ok(Some(current_file)) => {
                    ...
                    if current_file.is_file() {
                        ...
                        if let Ok(format) = guess {
                            ...
                            let loaded =
                                image::load_from_memory_with_format(raw.as_slice(), format);
                            if let Ok(dyn_img) = loaded {
                                *current_image.lock().unwrap() =
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    'restart: loop {
        ...
        for _ in 0..5 {
            match smol::block_on(Async::<TcpStream>::connect(SocketAddr::new(
        ...
        let mut app = App {
                ...
                stack: vec![(
                    ...
                    StateCallbacks {
                        shift_back: Box::new(|_, _| {
                            panic!("Shifted state away from base text area!")
                        }),
            ...
            messages: Some(StatefulList::with_items(
                Vec::new(),
                ...
                Some(res) => {
                    match res.unwrap() {
                        ...
                        ReadResult::SUCCESS(bytes_read) => {
                            let packet: Packet =
                                postcard::from_bytes(bytes_read.as_slice()).unwrap();
                            message_recv(
                                packet,
                                &mut app.messages,
                                &mut app.picker,
...
fn ui<B: Backend>(
    f: &mut Frame<B>,
    app_messages: &mut Option<StatefulList<Packet>>,
    app_zoom_x: u16,
    app_zoom_y: u16,
    app_image: &mut Option<ImageWithData>,
    app_state: &mut State,
    app_peers: &mut Option<StatefulList<Client>>,
) {
    if let Some(browser) = app_state.file_browser_mut() {
        ...
        let current_list = List::new(
            browser
                ...
                .iter()
                .map(|entry| {
                    ListItem::new(entry.clone().into_os_string().into_string().unwrap())
                        .style(Style::default().fg(Color::White).bg(Color::Black))
                })
        ...
        .block(
            Block::default()
                .title(browser.current_dir.to_str().unwrap())
                .borders(Borders::ALL),
        )
        .style(Style::default())
        .highlight_style(
        ...
        if let Ok(mut mutex) = browser.current_image.try_lock() {
            if let Some(img) = &mut *mutex {
                let img_layout = Block::default().borders(Borders::ALL).title(format!(
                    "Image {:?}",
                    browser
                        .current_dir_list
                        .current()
        ...
        if let Some(current_file) = browser.current_dir_list.current() {
            ...
            f.render_widget(paragraph, right_section[0]);
        } else {
            ...
            f.render_stateful_widget(
                messages.to_list(top_section[0].width.into()),
                top_section[0],
                &mut messages.state,
            );
            ...
            f.render_stateful_widget::<List>(
                <&StatefulList<Client> as Into<List>>::into(peers),
                top_section[1],
                &mut peers.state,
            );

            let app_textarea = if let Menu::MessageInput(x) = &mut app_state.stack[0].0 {
                x
            } else {
                panic!()
            };
    ...
    if let Menu::Popup(p) = &app_state.focus().0 {
        f.render_widget(
            Paragraph::new(p.content.as_str())
    ...
    match packet.message_type.clone() {
        ...
        MessageType::IMAGE(ref imgtype) => {
            ...
            let dyn_img = image::load_from_memory_with_format(
                messages.items.last().unwrap().data.as_slice(),
                imgtype.format,
            )
            .unwrap();
...
fn file_browser_input(
    inpt: Input,
    writer: &mut async_dup::Arc<Async<TcpStream>>,
    app: &mut App<'_>,
) {
    match inpt {
        _ if inpt == SHIFT_BACK => {
        ...
        Input { key: Key::Up, .. } => {
            ...
            browser
                .image_thread_sender
                .send(Some(browser.current_dir_list.current().unwrap().clone()))
                .unwrap();
        }
        Input { key: Key::Down, .. } => {
            let browser = app.state.file_browser_mut().unwrap();
        ...
        Input { key: Key::Left, .. } => {
            ...
            let _u = std::mem::replace(
                &mut app.state.file_browser_mut().unwrap().current_dir,
                PathBuf::from(current_dir.parent().unwrap()),
            );
            app.state.file_browser_mut().unwrap().current_dir_list = StatefulList::with_items(
                fs::read_dir(app.state.file_browser_mut().unwrap().current_dir.clone())
                    .unwrap()
                    .map(|entry| entry.unwrap().path())
                    .collect(),
                app.state
        ...
        Input {
            ...
            if selected.is_dir() {
                if fs::read_dir(selected.clone()).is_err() {
                    app.state.shift_state(
                        Menu::Popup(Popup {
                            title: String::new(),
                            content: "You don't have permission to access this directory!"
                                .to_string(),
                        }),
                        StateCallbacks {
                            shift_back: Box::new(|_, _| {}),
                ...
                if let Ok(format) = image::guess_format(raw.as_slice()) {
                    ...
                    if let Ok(_dyn_img) = loaded {
                        let packet = Packet {
                            ...
                            message_type: MessageType::IMAGE(ImageData { format, name }),
                            data: raw,
                        };
    ...
    match inpt {
        _ if inpt == SHIFT_BACK => {
            ...
            let _ = std::mem::replace(
                &mut app.peers,
                Some(if let Menu::PeerList(list) = app.state.shift_back() {
                    list
                } else {
                    panic!()
                }),
...
fn messages_list_input(inpt: Input, app: &mut App<'_>) {
    match inpt {
        _ if inpt == SHIFT_BACK => {
            ...
            let _ = std::mem::replace(
                &mut app.messages,
                Some(if let Menu::MessageList(list) = app.state.shift_back() {
                    list
                } else {
                    panic!()
                }),
...
fn messages_input(inpt: Input, writer: &mut async_dup::Arc<Async<TcpStream>>, app: &mut App<'_>) {
    match inpt {
        _ if inpt == SEND => 'a: {
                ...
                .deref()
            {
                "/file" => {
                    ...
                    app.state.shift_state(
                        Menu::FileBrowser(FileBrowser::new(current_path, &app.picker)),
                        StateCallbacks {
                            shift_back: Box::new(|_, _| {}),
                            shift_away: Box::new(|_| {}),
                            shift_to: Box::new(|_| {}),
        ...
        _ if inpt == SHIFT_TO_MESSAGES_LIST => {
            ...
            app.state.shift_state(
                Menu::MessageList(message_list.unwrap()),
                StateCallbacks {
                    shift_back: Box::new(|_, _| {}),
                    shift_away: Box::new(|menu| {
        ...
        _ if inpt == SHIFT_TO_PEERS => {
            ...
            app.state.shift_state(
                Menu::PeerList(peers_list.unwrap()),
                StateCallbacks {
                    shift_to: Box::new(|_| {}),
                    shift_away: Box::new(|_| {}),
...
mod tests {
    ...
    #[test]
    fn test_menu_ne() {
        assert_ne!(Menu::MessageInput(TextArea::default()), Menu::ImageView);
    }
}
